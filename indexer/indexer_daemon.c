...
#define LAST_ACTIVE_FILE "/tmp/vantage_last_active"
#define PENDING_DIRS_FILE "/tmp/vantage_pending_dirs"
#define CWD_FILE "/tmp/vantage_cwd"
...
long check_ram_buffer_size() {
    struct stat st;
    if (stat("/dev/shm/vantage_active_session", &st) == 0) {
